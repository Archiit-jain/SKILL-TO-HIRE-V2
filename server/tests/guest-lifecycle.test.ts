...
async function guestAnalysis(ctx: ReturnType<typeof testApp>, cookie: string) {
  const res = await analyseAs(request(ctx.app) as never).set("Cookie", cookie);
  return res;
}

async function signUpInBrowser(ctx: ReturnType<typeof testApp>, cookie: string, email: string) {
...
describe("D-6 guest lifecycle", () => {
  ...
  it("records a free-use marker with the first guest analysis and rejects the second", async () => {
    ...
    assert.equal(count(ctx.db, "SELECT COUNT(*) AS n FROM guest_free_use WHERE guest_id = ?", guestIdOf(cookie)), 1);
    assert.equal(count(ctx.db, "SELECT COUNT(*) AS n FROM guest_analyses WHERE guest_id = ?", guestIdOf(cookie)), 1);
  ...
  it("counts free use from the marker, not from guest_analyses", async () => {
    ...
    ctx.db.prepare("DELETE FROM guest_analyses WHERE guest_id = ?").run(guestIdOf(cookie));
    ...
    ctx.db.prepare("INSERT INTO guest_analyses (id, guest_id, result_json, created_at) VALUES ('legacy-row', ?, '{}', ?)").run(guestIdOf(other), daysAgo(1));
  ...
  it("moves the guest result into the account on sign-in, deletes the guest row and keeps the marker", async () => {
    ...
    assert.equal(count(ctx.db, "SELECT COUNT(*) AS n FROM guest_analyses WHERE guest_id = ?", guestIdOf(cookie)), 0, "claimed content is deleted");
    assert.equal(count(ctx.db, "SELECT COUNT(*) AS n FROM guest_free_use WHERE guest_id = ?", guestIdOf(cookie)), 1, "marker stays");
  ...
  it("returns unclaimed guest results only up to 30 days and purges older ones", async () => {
    ...
    ctx.db.prepare("UPDATE guest_analyses SET created_at = ? WHERE guest_id = ?").run(daysAgo(29), guestIdOf(recent));
    ctx.db.prepare("UPDATE guest_analyses SET created_at = ? WHERE guest_id = ?").run(daysAgo(31), guestIdOf(old));
    ...
    assert.equal(count(ctx.db, "SELECT COUNT(*) AS n FROM guest_analyses WHERE guest_id = ?", guestIdOf(old)), 0, "expired result purged");
    assert.equal(count(ctx.db, "SELECT COUNT(*) AS n FROM guest_analyses WHERE guest_id = ?", guestIdOf(recent)), 1, "recent result kept");
  ...
  it("doesn't claim a guest result older than 30 days", async () => {
    ...
    const cookie = guestCookie();
    assert.equal((await guestAnalysis(ctx, cookie)).status, 201);
    ctx.db.prepare("UPDATE guest_analyses SET created_at = ? WHERE guest_id = ?").run(daysAgo(31), guestIdOf(cookie));
    ...
    assert.equal(count(ctx.db, "SELECT COUNT(*) AS n FROM guest_analyses"), 0);
  });
  ...
  it("lets a browser analyse again once its marker is older than 365 days", async () => {
    ...
    const cookie = guestCookie();
    assert.equal((await guestAnalysis(ctx, cookie)).status, 201);
    ctx.db.prepare("UPDATE guest_free_use SET used_at = ? WHERE guest_id = ?").run(daysAgo(366), guestIdOf(cookie));
    ctx.db.prepare("UPDATE guest_analyses SET created_at = ? WHERE guest_id = ?").run(daysAgo(366), guestIdOf(cookie));
    const again = await guestAnalysis(ctx, cookie);
    assert.equal(again.status, 201, JSON.stringify(again.body));
    const marker = ctx.db.prepare("SELECT used_at FROM guest_free_use WHERE guest_id = ?").get(guestIdOf(cookie)) as { used_at: string };
    assert.ok(Date.now() - Date.parse(marker.used_at) < DAY, "marker renewed");
    assert.equal((await guestAnalysis(ctx, cookie)).status, 401);
  });

  it("the marker insert is the atomic gate: a second record for the same browser stores nothing", () => {
    const ctx = testApp();
    const id = guestIdOf(guestCookie());
    const now = new Date().toISOString();
    assert.equal(recordGuestAnalysis(ctx.db, id, "r1", "{}", now), true);
    assert.equal(recordGuestAnalysis(ctx.db, id, "r2", "{}", now), false);
    assert.equal(count(ctx.db, "SELECT COUNT(*) AS n FROM guest_analyses WHERE guest_id = ?", id), 1);
    assert.equal(count(ctx.db, "SELECT COUNT(*) AS n FROM guest_analyses WHERE id = 'r2'"), 0, "rolled back");
  });

  it("an expired marker is replaced atomically even when requests race", async () => {
    const ctx = testApp();
    const cookie = guestCookie();
    ctx.db.prepare("INSERT INTO guest_free_use (guest_id, used_at) VALUES (?, ?)").run(guestIdOf(cookie), daysAgo(400));
    const results = await Promise.all([guestAnalysis(ctx, cookie), guestAnalysis(ctx, cookie)]);
    assert.deepEqual(results.map((r) => r.status).sort(), [201, 401]);
  });
  ...
  it("account deletion leaves no analysis content behind, including claimed guest content", async () => {
    ...
    const id = (ctx.db.prepare("SELECT id FROM users WHERE email = 'delete-me@example.com'").get() as { id: string }).id;
    assert.equal(count(ctx.db, "SELECT COUNT(*) AS n FROM analyses WHERE user_id = ?", id), 2);

    assert.equal((await agent.delete("/api/account").set(CSRF).send({ currentPassword: "password-123" })).status, 204);
    assert.equal(count(ctx.db, "SELECT COUNT(*) AS n FROM analyses WHERE user_id = ?", id), 0);
    assert.equal(count(ctx.db, "SELECT COUNT(*) AS n FROM guest_analyses"), 0);
  ...
  it("a pre-P1 leftover (looks unclaimed after its account was deleted) disappears after 30 days", async () => {
    const ctx = testApp();
    const cookie = guestCookie();
    ctx.db.prepare("INSERT INTO guest_free_use (guest_id, used_at) VALUES (?, ?)").run(guestIdOf(cookie), daysAgo(40));
    ctx.db.prepare("INSERT INTO guest_analyses (id, guest_id, result_json, claimed_by, created_at) VALUES ('leftover', ?, '{\"id\":\"leftover\"}', NULL, ?)").run(guestIdOf(cookie), daysAgo(40));
    assert.equal((await request(ctx.app).get("/api/analyses/latest").set("Cookie", cookie)).body.result, null);
    assert.equal(count(ctx.db, "SELECT COUNT(*) AS n FROM guest_analyses WHERE id = 'leftover'"), 0);
  });
});
